 ...
 */
public class ChatSDKBaseActivity extends FragmentActivity implements ChatSDKBaseActivityInterface {
    ...
    protected void createAndOpenThreadWithUsers(final String name, BUser...users){
        Log.e("name",name);
        getNetworkAdapter().createThreadWithUsers(name, users)
                .done(new DoneCallback<BThread>() {
                    ...
                    public void onDone(final BThread thread) {
                        ...
                        if (isOnMainThread()) {
                            Log.e("thread id",thread.getEntityID());
                            ...
                            Firebase childref=ref.child("group-name");
