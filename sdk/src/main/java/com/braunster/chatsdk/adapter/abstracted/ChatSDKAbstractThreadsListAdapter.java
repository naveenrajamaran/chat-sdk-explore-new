...
import com.braunster.chatsdk.activities.LocationSharing;
...
static int k=0;
    public static List<BMessage> message;

    public static  String it[]=new String[2];
    protected Activity mActivity;
    static  String displayName="";
    protected List<E> threadItems = new ArrayList<E>();
    protected List<E> listData = new ArrayList<E>();
    public  static List<String> adding= new ArrayList<String>();
    ...
    protected ThreadListItemMaker<ThreadListItem> getDefaultMaker(){
        return new ThreadListItemMaker<ThreadListItem>() {
            ...
            public ThreadListItem fromBThread(BThread thread) {
                ...
                final List<BUser> users = thread.getUsers();
                String url  = thread.threadImageUrl();

                Firebase ref = new Firebase("https://revsmarttech.firebaseio.com/testRoot/threads/"+thread.getEntityID()+"/details");
                ref.addChildEventListener(new ChildEventListener() {

                    @Override
                    public void onChildAdded(DataSnapshot dataSnapshot, String s) {

                        if(dataSnapshot.getKey().equalsIgnoreCase("group-name")) {
                            String s1=(String)dataSnapshot.getValue();

                                LocationSharing.groupNames.add((String) dataSnapshot.getValue());

                                setDisplayName(dataSnapshot.getValue() + " ");
                ...
                //  displayName = value;
                thread.displayName(users);

                if(users.size()==2) {
                    String s=displayName;
                    displayName="";
                        LocationSharing.groupNames.add((String) thread.displayName(users));


                    return new ThreadListItem(thread.getEntityID(),
                            StringUtils.isEmpty(s) ?thread.displayName(users) : thread.displayName(users),
                            url,
                            data[1],
                            data[0],
                            users.size(),
                    ...
                    return new ThreadListItem(thread.getEntityID(),
                            StringUtils.isEmpty(s) ? thread.displayName(users) : s,
                            url,
                            data[1],
                            data[0],
                            users.size(),
                    ...
                    List<BUser> users1 = thread.getUsers();
                    String displayName1 = thread.displayName(users1);
                    return new ThreadListItem(thread.getEntityID(),
                            StringUtils.isEmpty(displayName1) ? displayName1 : displayName1,
                            url,
                            data[1],
                            data[0],
                            users.size(),
